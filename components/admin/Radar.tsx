...
import type { Ratings } from './config';
...
export default function Radar({ ratings, size = 190 }: RadarProps) {
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', overflow: 'visible' }}>
      <defs>
        <linearGradient id="rf" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#1e6cb6" stopOpacity=".18" />
          <stop offset="100%" stopColor="#7db8e8" stopOpacity=".08" />
        </linearGradient>
      </defs>
      {[1,2,3,4,5,6].map(lv =>
      ...
      {CATS.map((c, i) => {
        ...
        return (
          <circle
            key={c.key} cx={x} cy={y}
            r={isHov ? 8 : 5.5}
            fill={s.c} stroke="#fff" strokeWidth="2.5"
      ...
      {CATS.map((c, i) => {
        const [x, y] = pt(i, 8);
        return <text key={c.key + 't'} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="#94a3b8" fontSize="13">{c.icon}</text>;
      })}
      ...
      {hovered && (() => {
        ...
        return (
          <g key="tip" style={{ pointerEvents: 'none' }}>
            <rect x={tx} y={ty} width={tw} height={th} rx="6" fill={s.c} opacity=".93" />
