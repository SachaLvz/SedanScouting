...
export default function ScoutShadowTeamPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/shadow-categories?ownerId=${ownerId.current}`).then(r => r.json()),
      fetch(`/api/shadow-team?ownerId=${ownerId.current}`).then(r => r.json()),
  ...
  const handleCreateTeam = async (name: string, categoryId?: string | null) => {
    const res = await fetch('/api/shadow-team', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ownerId: ownerId.current, name, formation: '4-3-3', categoryId: categoryId ?? null }),
    });
    const team = await res.json();
  ...
  const handleCreateCategory = async (name: string) => {
    const res = await fetch('/api/shadow-categories', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ownerId: ownerId.current, name }),
    });
